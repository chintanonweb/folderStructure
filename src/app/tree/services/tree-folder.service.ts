...
export class TreeFolderService {
  ...
  treeFolderData: TreeNodeModel[] = []

  // store parents node of treeDirectorey using behaviour subject and bydefault parameter is treeFolderData array
  treeData: BehaviorSubject<TreeNodeModel[]> = new BehaviorSubject<TreeNodeModel[]>(this.treeFolderData);
  ...
  // if any node want to store the value in it's children then update the value of that node and emit next value
